...
interface StudyNotesViewProps {
  studyNotes: DocumentNote;
  onRegenerate?: () => void;
  isRegenerating?: boolean;
}

const StudyNotesView: React.FC<StudyNotesViewProps> = ({
  studyNotes,
  onRegenerate,
  isRegenerating = false,
}) => {
  const handleCopy = () => {
    navigator.clipboard.writeText(studyNotes.content);
    ...
    <Card className="bg-gray-800/50 border-gray-700">
      <CardContent className="pt-6">
        <div className="flex justify-between items-start mb-4">
          <h3 className="text-lg font-semibold text-gray-200">{studyNotes.title}</h3>
        ...
        <div className="text-sm text-gray-400 mb-2">
          Generated on {studyNotes.createdAt}
        </div>
        ...
        <div className="prose prose-sm max-w-none dark:prose-invert prose-headings:text-gray-200 prose-p:text-gray-300 prose-strong:text-gray-200 prose-ul:text-gray-300 mt-4">
          {isRegenerating ? (
            <div className="flex flex-col items-center py-4">
              ...
              <p className="text-gray-300">Regenerating study notes...</p>
            </div>
          ) : (
            <div className="prose prose-sm max-w-none dark:prose-invert text-white prose-headings:text-white prose-p:text-gray-200 prose-strong:text-white prose-a:text-blue-400 prose-ul:text-gray-200 prose-ol:text-gray-200 prose-li:text-gray-200">
              <ReactMarkdown>{studyNotes.content}</ReactMarkdown>
            </div>
          )}
        </div>
      </CardContent>
